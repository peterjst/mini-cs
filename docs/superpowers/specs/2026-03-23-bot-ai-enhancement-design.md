...
**Difficulty scaling on peripheral awareness**: If the player is in the outer 30° of the cone (angle between 30°–60° from center), reaction time is modified:
- Easy: +50% reaction delay for peripheral detections
- Normal: +25% reaction delay
- Hard: +10% reaction delay
- Elite: No penalty
...
- **Sightline quality**: Raycast from the waypoint in several directions. More open sightlines = higher score. Bots prefer positions with visibility, not dead ends.
...
- **Time since last visited**: Waypoints not visited recently score higher. Prevents looping and creates natural clearing/sweep patterns.
- **Distance from other bots**: Waypoints far from teammates score slightly higher. Prevents clumping and creates map spread.
...
Uses existing waypoint arrays from maps. Scoring happens at waypoint selection time only, not every frame.
...
- Personality roll passes: Cautious 60%, Balanced 30%, Aggressive 10%
...
- **Spring the trap**: When the player enters FOV and reaction delay passes, transition to ATTACK with a reduced reaction delay multiplier (bot was pre-aimed and expecting the player)
...
- ATTACK: Player enters FOV and reaction passes
- PATROL: Timeout expires
- RETREAT/ATTACK: Takes damage (depending on HP and whether attacker is visible)
