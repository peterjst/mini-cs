...
| `startAmbient(mapName)` | Per-map ambient soundscape (looped, 2s fade-in to gain 0.04). Dust: desert wind (bandpass 250Hz + LFO). Office: 120Hz hum (gain 0.18) + AC highpass noise (gain 0.1). Warehouse: 60Hz drone (gain 0.3) + metallic pings (gain 0.1). Bloodstrike: crowd noise (bandpass 400Hz, gain 0.4 + LFO). Italy: wind (gain 0.35) + distant bells. Aztec: insect noise (4kHz) + bird chirps. Default: subtle wind (gain 0.4). All maps normalized to similar perceived volume. Plays in all game modes including Tour. |
