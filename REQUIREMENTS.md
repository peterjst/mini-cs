...
- **Bot respawn validation (Deathmatch, Gun Game) and boss minion summoning:** any code that places an enemy at `origin + random offset` MUST validate the candidate position against walls before spawning. Validation is provided by `GAME._findValidSpawn(originX, originZ, walls, opts)`, which retries up to 20 random angles within `[opts.minOff, opts.maxOff]` and checks both `_isSpawnClear` (cardinal-direction clearance) and `_hasLineOfSight` (no wall between origin and spawn). Optional `opts.minPlayerDist` + `opts.playerPos` reject spawns within that radius of the player. Returns `null` after retries, in which case the caller falls back to a known-safe origin (waypoint center for respawns, boss position for minions). Bypassing this validator is what previously let enemies appear inside Bloodstrike's enclosed inner block when waypoints/boss positions sat near the inner walls
- **Helpers:** `GAME._mapHelpers.randomSpawnInZone(zone, walls)`, `GAME._mapHelpers.pickSpawnZone(zones, label, enemies)`, `GAME._findValidSpawn(originX, originZ, walls, opts)`
...
- **Walls**: Outer perimeter walls (tan concrete 0xb8a080) and inner block walls (darker tan 0x9a8060). N/S inner walls extended by 2×wallThickness (42 wide instead of 40) so they overlap E/W walls at corners, preventing diagonal gap that could let entities slip inside the inner block with **thick horizontal trim bands** (0x7a6850, height 0.35) at y=1.8 and y=4.2, thin middle trim (0x857460) at y=3.0. **Color banding** between trims: darker bottom band (0xa08868) floor-to-1.6, lighter top band (0xc8b898) 4.4-to-ceiling. Baseboards (0x706050) on all walls
...
- **Decoration**: Horizontal wall pipes along outer walls, vertical corner pipes, blood splatters/stains on walls and floor, scattered rubble via `P.Rubble()`, yellow warning stripes near corners. **Inner wall gap detail**: junction boxes via `P.Junction()` on side inner walls, horizontal mounted pipe runs (darkMetal) at ~5m height on N/S inner walls, faded poster/sign patches (muted tan/green), water stain drips below pipes. **Outer wall gap detail**: ventilation grates (dark 0x3a3a3a) centered on each outer wall at y=5, metal bracket strips (darkMetal) at y=5.5, paint fade patches (subtle tan color variation). **Platform column detail**: bolted base plates (darkMetal) and pipe clamps (metal) on front-facing support columns
...
- Waypoints: 23 points along open outdoor paths, doorway entrances, alleys, piazza, and 2nd floors of Buildings A and B — outdoor navigable space plus elevated interior positions
- **Central Piazza**: Open square with fountain via `P.Fountain()`, cobblestone path markings
- **Building A (North)**: 2-story accessible building (x=-8..4, z=-25..-12). Wide interior stairs (width 2.0) from ground to upper floor with stairwell opening (x[-8,-4], z[-17,-12]) and iron railings around the opening. Terracotta roof with eave overhang. Shutters, flower boxes on south face. Balcony overlooking south with iron railing. Door lintel over south entry. **2nd floor (y=3.5)**: Split floor slab (back section + front-right section) leaving stairwell opening on left side. Wood plank flooring, 2 desks with monitors overlooking piazza, filing cabinet cluster against north wall, low bookshelf for interior cover, window sill (h=1.0) in south wall gap, window cutout in east wall facing alley
- **Building B (East, T-side)**: Large 2-story apartment block (x=10..22, z=-20..5). Ground+upper floor with wide interior stairs (width 2.0, x=19) and stairwell opening (x[17,22], z[-12,-7]) with iron railings. **South wall doorway** (x=14..18 gap) with stone lintel above entry. West balcony with iron railing over alley. Shutters on west face. Terracotta roof with eave. **2nd floor (y=3.5)**: Split floor slab (back + left-of-stairwell + front sections) leaving stairwell opening on right side near back room. Wood plank flooring, partition wall (h=3) dividing into front/back rooms. Front room: table with chairs, crate stack cover, window sills in south wall overlooking piazza, west wall windows facing alley, cover crate beside balcony railing. Back room: bookcase shelf, desk near north window overlooking bombsite A, west wall window facing alley
...
- **Perimeter Wall Facades**: Window frames with colored shutters on all 4 walls — north (3 windows: green/blue/terracotta shutters), south (3 windows: terracotta/green/blue), east (2 windows: green/terracotta), west (2 windows: blue/green). Each window has dark recess, sandstone lintel + sill, paired shutters. Flower box ledges with greenery under select windows (north x=-15, south x=5, east z=-12). Facade color variation plaster patches (warm orange 0xd4a070, cream 0xddc8a8, pink 0xd8a888) simulating different building fronts. Additional clothesline with laundry between Buildings A and B
...
3. **ATTACK**: Burst-fire at player with 6 combat movement sub-behaviors (strafe, push, hold, retreat-fire, rush-to-cover, reposition) selected by personality-weighted random with context modifiers. LOS grace period of 0.5s prevents disengagement on momentary obstruction — bot continues firing at last known position during grace. Combat movement resets on state exit. **Stale position failsafe**: Every `staleThreshold` seconds (easy: 6.0s, normal: 4.0s, hard: 2.5s, elite: 1.8s), if the bot hasn't moved more than 1 unit, a forced reroll of the combat move is triggered (HOLD is excluded from forced rerolls — reassigned to STRAFE). Stale tracking resets when entering ATTACK from another state.
4. **INVESTIGATE**: Move to last-known player position when LOS lost, look around before resuming patrol. Look-around duration is difficulty-scaled: Easy 3–4s, Normal 2.5–3.5s, Hard 1.5–2s, Elite 1–1.5s. On hard/elite, bot moves in a slow circle while looking around instead of standing still. Also triggered by sound awareness. **Stuck detection**: Actively turns away from walls; falls back to PATROL if bot hasn't moved >1 unit in 1.5 seconds while navigating to investigate position
...
- **Reload auto-strafe**: Bot strafes while reloading in ATTACK state (does not apply during TAKE_COVER)
...
- **Patrol pause**: Difficulty-scaled multiplier on personality base pause. Easy: 100%, Normal: 70%, Hard: 30%, Elite: 0%
...
- **Balaclava**: Near-black hemisphere dome `SphereGeometry(0.31, 10, 7, ...)` over top of head (knit cap), + partial sphere `SphereGeometry(0.22, 10, 8, 0, Math.PI*2, Math.PI*0.45, Math.PI*0.35)` face mask at y=2.08, scaled (1.0, 1.1, 0.9) — covers mouth/nose, leaves eyes exposed
...
| `startAmbient(mapName)` | Per-map ambient soundscape (looped, 2s fade-in to gain 0.04). Dust: desert wind (bandpass 250Hz, gain 0.4 + LFO). Office: 120Hz hum (gain 0.08) + AC highpass noise (gain 0.04). Warehouse: 60Hz drone (gain 0.3) + metallic pings (gain 0.1). Bloodstrike: indoor arena — 100Hz electrical hum (gain 0.1) + lowpass ventilation noise (800Hz, gain 0.3). Italy: wind (gain 0.35). Aztec: insect noise (4kHz) + bird chirps. Default: subtle wind (gain 0.4). All maps normalized to similar perceived volume. Plays in all game modes including Tour. |
...
### Screens
- **Menu screen**:
  - **Live 3D background**: Random map rendered behind menu using `GAME.buildMap()` and `renderWithBloom()` pipeline. New random map built on each menu entry via `_buildMenuScene()` / `GAME.buildMenuScene()`. Birds fly in background. Ambient sound plays for the background map (deferred until first user gesture if AudioContext is suspended). First-person weapon model is hidden during menu flythrough.
  ...
  - **Vertical scroll when content exceeds viewport**: `#menu-screen` uses `overflow-y: auto` with `justify-content: safe center`, so content is vertically centered when it fits and anchors to top (remaining fully reachable via scroll) when it does not. This ensures the "back to modes" link and footer stay accessible when taller config panels (e.g. Competitive TEAM mode's extra Objective/Side rows) grow the card beyond viewport height. The background overlay (`#menu-bg`) is `position: fixed` so it stays pinned to the viewport during scroll.
  ...
  - 2x2 mode card grid: Competitive (blue accent/primary), Survival, Gun Game, Deathmatch
    ...
    - **Competitive card** has additional team mode options:
      - Boss Fight button (`#comp-boss-btn`): crimson-themed `.boss-fight-btn` variant of `.mode-start-btn` in Competitive card; skips to boss round with $10,000 starting money; hidden when SOLO is not the selected mode (Team mode has no boss)
...
- **Tour map selection** (full-screen overlay, z-index 30):
  ...
  - Clicking a map dismisses the tour panel, then starts tour mode via `_fadeMenuAndStart()` — triggers fullscreen + landscape lock on mobile, identical to other game modes
- **Tour mode HUD**:
